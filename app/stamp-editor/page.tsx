...
export default function StampEditorPage() {
  ...
  const syncFromObj = useCallback((obj: any) => {
    ...
    if (st === 'mseg') {
      ...
      setSelTrapRx(obj._triangleRadius ?? 0);
    } else if (st === 'arc') {
  ...
  const applyTriangleRadius = useCallback((radius: number) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const points: { x: number; y: number }[] = (old as any)._trianglePoints ?? [
        ...
        <div style={S.panel}>
          ...
          {selectedShapeType === 'triangle' && !isMseg && (
            <>
              <div style={{ height: 1, background: 'var(--border)' }} />
              <div>
                <div style={S.label}>三角形 角の丸み</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <button onClick={() => applyTriangleRadius(Math.max(0, selTrapRx - 2))} style={S.actionBtn()}><Minus size={12} /></button>
                  <span style={{ flex: 1, textAlign: 'center', fontSize: 12, color: 'var(--text)' }}>{selTrapRx}</span>
                  <button onClick={() => applyTriangleRadius(selTrapRx + 2)} style={S.actionBtn()}><Plus size={12} /></button>
                </div>
              </div>
            </>
          )}
